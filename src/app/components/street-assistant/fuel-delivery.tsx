...
import { ChevronLeft, CreditCard, Wallet, Smartphone, CheckCircle2, Clock, Star, Fuel as FuelIcon, Droplet, MapPin } from 'lucide-react';
import { useNavigate } from 'react-router';
import { BottomNav } from '../bottom-nav';
import { streetAssistantProviders, fuelTypes, UserCar } from '../../data/mock-data';
import { LocationPicker } from './location-picker';
import { CarSelector } from './car-selector';

export function FuelDelivery() {
  const navigate = useNavigate();
  const [step, setStep] = useState<'location' | 'fuel' | 'payment' | 'provider' | 'waiting'>('location');
  ...
  const providers = streetAssistantProviders['fuel-delivery'];
  ...
  const calculateFuelCost = () => {
    ...
    return selectedFuel.price * fuelQuantity;
  };

  const calculateTotal = (deliveryCost: number) => {
    return calculateFuelCost() + deliveryCost;
  };

  const handleNext = () => {
    if (step === 'location' && currentLocation && selectedCar) {
      setStep('fuel');
    } else if (step === 'fuel' && fuelType && fuelQuantity) {
      setStep('payment');
  ...
  const canProceed = () => {
    ...
    if (step === 'fuel') return fuelType && fuelQuantity;
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#F2F5FB] via-white to-[#FEFBF3] pb-24">
      ...
      <div className="bg-gradient-to-r from-[#2952AB] to-[#3B6EC9] px-4 pt-12 pb-6">
        <div className="max-w-md mx-auto">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="font-medium text-white">Fuel Delivery</h1>
              <p className="text-sm text-white/80">Gas delivered to your location</p>
            </div>
          </div>

          {/* Progress Steps */}
          <div className="flex items-center gap-2 mt-6">
            {['location', 'fuel', 'payment', 'provider', 'waiting'].map((s, idx) => (
              <div key={s} className="flex items-center flex-1">
                <div className={`h-1 flex-1 rounded-full ${
                  step === s || ['location', 'fuel', 'payment', 'provider', 'waiting'].indexOf(step) > idx
                    ? 'bg-[#C69815]'
                    : 'bg-white/30'
                }`} />
              </div>
      ...
      <div className="max-w-md mx-auto px-4 mt-6 space-y-6">
        ...
        {step === 'fuel' && (
          <>
            <div>
              <h3 className="text-sm font-medium text-[#2952AB] mb-3">Select Fuel Type</h3>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Droplet size={20} className="text-[#2952AB]" strokeWidth={1.5} />
                        <span className="font-medium text-gray-900">{fuel.name}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="font-semibold text-[#2952AB]">${fuel.price}/gal</span>
                        {fuelType === fuel.id && (
            ...
            <div>
              <h3 className="text-sm font-medium text-[#2952AB] mb-3">Select Quantity (Gallons)</h3>
              <div className="grid grid-cols-4 gap-2">
                {quantities.map((qty) => (
                  <button
                    key={qty}
                    onClick={() => setFuelQuantity(qty)}
              ...
              <div className="mt-4 bg-gradient-to-r from-[#FEF8E7] to-[#E4ECF7] rounded-[10px] p-4 border border-[#C69815]/20">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[#2952AB]/80">Fuel Cost</span>
                  <span className="font-semibold text-[#2952AB]">${calculateFuelCost().toFixed(2)}</span>
                </div>
                <p className="text-xs text-[#2952AB]/60 mt-2">+ Delivery fee varies by distance</p>
              </div>
            </div>
          </>
        )}
        ...
        {step === 'provider' && (
          <div>
            <h3 className="text-sm font-medium text-[#2952AB] mb-3">Available Providers</h3>
                  ...
                  >
                    <div className="flex items-start gap-3 mb-3">
                      <div className="w-12 h-12 bg-gradient-to-br from-[#2952AB]/10 to-[#C69815]/10 rounded-full flex items-center justify-center flex-shrink-0">
                        <FuelIcon size={20} className="text-[#2952AB]" strokeWidth={1.5} />
                      </div>
                      ...
                      <div className="text-right">
                        <div className="font-semibold text-[#2952AB]">${totalCost.toFixed(2)}</div>
                        {selectedProvider === provider.id && (
                    ...
                    <div className="mt-2 text-xs text-gray-500 bg-[#FEF8E7] px-2 py-1 rounded">
                      Fuel: ${calculateFuelCost().toFixed(2)} + Delivery: ${provider.baseCost}
                    </div>
                  </button>
                );
              })}
      ...
      {step !== 'waiting' && (
        <div className="fixed bottom-20 left-0 right-0 bg-gradient-to-t from-white via-white to-transparent px-4 pt-4 pb-4">
          <div className="max-w-md mx-auto">
            ...
            >
              {step === 'location' && 'Continue to Fuel Selection'}
              {step === 'fuel' && 'Continue to Payment'}
              {step === 'payment' && 'Continue to Select Provider'}
              {step === 'provider' && 'Request Fuel Delivery'}
            </button>
          </div>
        </div>
      )}
