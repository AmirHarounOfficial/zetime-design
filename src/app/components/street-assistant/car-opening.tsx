...
import { ChevronLeft, CreditCard, Wallet, Smartphone, CheckCircle2, Clock, Star, KeyRound, Upload, X, MapPin } from 'lucide-react';
...
export function CarOpening() {
  const navigate = useNavigate();
  const [step, setStep] = useState<'location' | 'proof' | 'payment' | 'provider' | 'waiting'>('location');

  const [currentLocation, setCurrentLocation] = useState('');
  const [selectedCar, setSelectedCar] = useState<UserCar | null>(null);
  const [carPlateImage, setCarPlateImage] = useState<string | null>(null);
  const [licenseImage, setLicenseImage] = useState<string | null>(null);
  const [carLicenseImage, setCarLicenseImage] = useState<string | null>(null);
  const [paymentMethod, setPaymentMethod] = useState('');
  const [selectedProvider, setSelectedProvider] = useState<string | null>(null);
  const [waitingTime, setWaitingTime] = useState(60);

  const providers = streetAssistantProviders['car-opening'];
  ...
  const handleImageUpload = (type: 'plate' | 'license' | 'carLicense') => {
    // Simulate image upload
    const placeholder = 'https://via.placeholder.com/150?text=Uploaded';
    if (type === 'plate') setCarPlateImage(placeholder);
    if (type === 'license') setLicenseImage(placeholder);
    if (type === 'carLicense') setCarLicenseImage(placeholder);
  };

  const handleNext = () => {
    if (step === 'location' && currentLocation && selectedCar) {
      setStep('proof');
    } else if (step === 'proof' && carPlateImage && licenseImage && carLicenseImage) {
      setStep('payment');
  ...
  const canProceed = () => {
    ...
    if (step === 'proof') return carPlateImage && licenseImage && carLicenseImage;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F2F5FB] via-white to-[#FEFBF3] pb-24">
      ...
      <div className="bg-gradient-to-r from-[#2952AB] to-[#3B6EC9] px-4 pt-12 pb-6">
        <div className="max-w-md mx-auto">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="font-medium text-white">Locked Car Opening</h1>
              <p className="text-sm text-white/80">Emergency lockout service</p>
            </div>
          </div>

          {/* Progress Steps */}
          <div className="flex items-center gap-2 mt-6">
            {['location', 'proof', 'payment', 'provider', 'waiting'].map((s, idx) => (
              <div key={s} className="flex items-center flex-1">
                <div className={`h-1 flex-1 rounded-full ${
                  step === s || ['location', 'proof', 'payment', 'provider', 'waiting'].indexOf(step) > idx
                    ? 'bg-[#C69815]'
                    : 'bg-white/30'
                }`} />
              </div>
      ...
      <div className="max-w-md mx-auto px-4 mt-6 space-y-6">
        ...
        {step === 'proof' && (
          <div className="space-y-4">
            <div className="bg-gradient-to-r from-[#FEF8E7] to-[#E4ECF7] rounded-[10px] p-4 border border-[#C69815]/20">
              <p className="text-sm text-[#2952AB]">
                <strong>Important:</strong> For security, we require proof that you own this vehicle.
              </p>
            </div>

            {/* Car with Plate */}
            <div>
              <h4 className="text-sm font-medium text-[#2952AB] mb-2">Car with License Plate Visible</h4>
              {!carPlateImage ? (
                <button
                  onClick={() => handleImageUpload('plate')}
                  className="w-full border-2 border-dashed border-[#C2D1E8] rounded-[10px] p-8 flex flex-col items-center justify-center hover:border-[#2952AB] transition-all"
                >
                  <Upload size={32} className="text-[#7A9ACB] mb-2" strokeWidth={1.5} />
                  <span className="text-sm text-gray-600">Upload Photo</span>
                </button>
              ) : (
                <div className="relative">
                  <div className="w-full h-32 bg-gray-100 rounded-[10px] flex items-center justify-center">
                    <CheckCircle2 size={32} className="text-green-500" strokeWidth={1.5} />
                  </div>
                  <button
                    onClick={() => setCarPlateImage(null)}
            ...
            {/* Driver's License */}
            <div>
              <h4 className="text-sm font-medium text-[#2952AB] mb-2">Driver's License</h4>
              {!licenseImage ? (
                <button
            ...
            {/* Car Registration */}
            <div>
              <h4 className="text-sm font-medium text-[#2952AB] mb-2">Car Registration/License</h4>
              {!carLicenseImage ? (
                <button
                  onClick={() => handleImageUpload('carLicense')}
                  className="w-full border-2 border-dashed border-[#C2D1E8] rounded-[10px] p-8 flex flex-col items-center justify-center hover:border-[#2952AB] transition-all"
                >
                  <Upload size={32} className="text-[#7A9ACB] mb-2" strokeWidth={1.5} />
                  <span className="text-sm text-gray-600">Upload Photo</span>
                ...
                <div className="relative">
                  ...
                  <button
                    onClick={() => setCarLicenseImage(null)}
                    className="absolute top-2 right-2 w-8 h-8 bg-red-500 text-white rounded-full flex items-center justify-center"
                  >
                    <X size={16} strokeWidth={2} />
                  </button>
        ...
        {step === 'provider' && (
          <div>
            <h3 className="text-sm font-medium text-[#2952AB] mb-3">Available Locksmiths</h3>
                ...
                >
                  <div className="flex items-start gap-3 mb-3">
                    <div className="w-12 h-12 bg-gradient-to-br from-[#2952AB]/10 to-[#C69815]/10 rounded-full flex items-center justify-center flex-shrink-0">
                      <KeyRound size={20} className="text-[#2952AB]" strokeWidth={1.5} />
                    </div>
                    ...
                    <div className="text-right">
                      <div className="font-semibold text-[#2952AB]">${provider.baseCost}</div>
                      {selectedProvider === provider.id && (
        ...
        {step === 'waiting' && (
          <div className="text-center py-8">
            ...
            <h3 className="text-xl font-medium text-[#2952AB] mb-2">Verifying Documents</h3>
            <p className="text-gray-500 mb-4">
              Provider is reviewing your documents. This may take up to 1 minute.
            </p>
      ...
      {step !== 'waiting' && (
        <div className="fixed bottom-20 left-0 right-0 bg-gradient-to-t from-white via-white to-transparent px-4 pt-4 pb-4">
          <div className="max-w-md mx-auto">
            ...
            >
              {step === 'location' && 'Continue to Upload Proof'}
              {step === 'proof' && 'Continue to Payment'}
              {step === 'payment' && 'Continue to Select Provider'}
              {step === 'provider' && 'Request Unlock Service'}
            </button>
          </div>
        </div>
      )}
